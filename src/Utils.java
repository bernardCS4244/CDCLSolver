...
public class Utils {
	...
	public static Literal findUnitLiteral(List<Literal> clause, int[] literalTracker, HashMap<Integer, Literal> solution) {
		//return null if not unit clause else, returns the only unassigned literal
		int numAssigned = 0;
		...
		if (clause.size() == 1) {
			...
			for (Literal literal: clause) {			
				...
				if (literalTracker[literal.get()] != -1) {
					numAssigned++;
					//else not assigned, check if there is only one literal unassigned left the whole value of clause if false
				} else {
					unassigned = literal;
				}
					
				if ((clause.size() - numAssigned == 1) && !isClauseTrue(clause, literalTracker, solution) && unassigned != null) {
					return unassigned;
				}
			}
		}
