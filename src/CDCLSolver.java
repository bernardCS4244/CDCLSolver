...
public class CDCLSolver {
	...
	public CDCLSolver(int numLiterals, List<Clause> clauses) {
		...
		//keep track of which literals have been assigned. int indicated the decision level when literal is assigned
	...
	private Action decide() {
		...
		for(int i=1; i<=numLiterals; i++) {
			//checks if literal is not assigned and there are still values not used in decision making for that literal
	...
	private Action handleUnitClause(Literal unitLit) {
		...
		if(literalTracker[unitLit.get()] == -1) {
			//if literal is negative, the assigned value must be false. same for positive literal
			addToSolution(unitLit, Action.UNIT_PROPAGATION);
			return Action.UNIT_PROPAGATION;

			//if the assigned value in solution is different from that in the clause = conflict
	...
	private Clause learnNewClause() {
		...
		//iterate through literals assigned via decision and has a path to the conflict
		for (int i=1; i<=numLiterals; i++) {
...
//				}
				...
				//set the literal value in the new clause the complement of the current assignment
				if (literal.getValue()) {
	...
	private void backTrack(Clause learntClause) {
		...
		//remove all literal assigned greater than the backtracked level from solution, implication graph, decisionTracker and literalTracker
		for(int i=1; i<=numLiterals; i++) {
			if (literalTracker[i] >= backTrackToLevel) {
				...
				//remove assignment from literals greater than decision level or if the literal is not in the learnt clause
			...
			//remove outgoing edges which are implied greater than or equal to backTrackLevel
			if (literalTracker[i] < backTrackToLevel) {
	...
	//propagation the implication graph using all the other literals in the clause
		...
		for(Literal literal : clausesMasterList.get(clauseIndex).getDisjunctiveClause()) {
			if (literal.get() != lastAssignedLiteral && type == Action.UNIT_PROPAGATION) {
				if (!implicationGraph.get(literal.get()).contains(lastAssignedLiteral)) {
					...
					reverseImplicationGraph.get(lastAssignedLiteral).add(new ImplicationDetails(literal.get(), clauseIndex));
				}
